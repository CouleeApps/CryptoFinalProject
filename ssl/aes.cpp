...
vector<vector<u8> > add_round_key(vector<vector<u8> > state_array, vector<vector<u8> > key){
	vector<vector<u8> > copy(state_array);
	for(size_t i = 0; i < state_array.size(); i++){
		for(size_t j = 0; j < state_array[i].size(); j++){
...
vector<u8> aes_decrypt(u8 message[AES_BLOCK_SIZE], u8 key[AES_BLOCK_SIZE]){
...
vector<u8> aes_encrypt(u8 cipher[AES_BLOCK_SIZE], u8 key[AES_BLOCK_SIZE]){
